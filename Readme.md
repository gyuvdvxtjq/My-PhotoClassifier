...
| `git_hub_repo_url` | `string` | **GitHub 仓库地址，格式为 `owner/repo`。**        | `MyUser/PhotoArchive`         |
| `github_token`     | `string` | **GitHub 个人访问令牌 (PAT)，需具备 `repo` 写入权限。** | `ghp_xxxxxxxxxxxx`            |
| `github_dir`       | `string` | 图片上传到 GitHub 仓库中的目标父目录。                  | `classified_photos`           |
...
```json
{
  ...
  "llm_type": "gemini",
  "model_custom_url": "",
  "git_hub_repo_url": "YourUsername/YourRepoName",
  "github_token": "YOUR_GITHUB_PAT_TOKEN",
  "github_dir": "sorted_archive",
...
2. **GitHub 路径：** 程序会将图片上传到 `$GitHubDir/$类别/$文件名` 的路径下，并自动创建目录。
3. **JSON 返回：** **PhotoClassifier** 依赖 LLM 返回精确的 JSON 格式（`{"cate": ["tag1", "tag2"]}`），如果模型返回格式错误，分类步骤将失败。
